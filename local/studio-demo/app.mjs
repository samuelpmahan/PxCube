...
function intro(title,description){return `<div class="intro"><span class="eyebrow">${config.mode==='bag'?'YOUR NEXT ROUND':'ON THE COURSE · DISCSPOTLIGHT'}</span><h1>${title}</h1><p class="muted">${description}</p></div>`;}
...
async function drawBag(){
  const context=model.context,view=await model.queryShelf({query}),all=rows();
...
async function drawCreate(){
  ...
  const all=rows(),selected=all.find(row=>row.address===context.selectedDisc);
  $('app').innerHTML=intro('Give this disc the spotlight.','Choose a specimen, shape its presentation, and keep a graphic you can take with you.')+`<div class="workspace two"><aside class="panel"><h2>Choose your material</h2><label>Physical copy<select id="graphic-disc">${all.map(row=>`<option value="${esc(row.address)}" ${row.address===context.selectedDisc?'selected':''}>${esc(row.seed.manufacturer+' · '+row.seed.name+' · '+row.disc.plastic+' · '+row.disc.weight+' g')}</option>`).join('')}</select></label><div class="art">${art(selected)}</div><p class="notice">The disc’s facts and retained painting stay intact. These controls change this graphic’s design.</p><label>Design<select id="preset">${[['spotlight','Spotlight'],['showcase','Showcase'],['broadcast','Lower third']].map(([v,label])=>`<option value="${v}" ${v===design.preset?'selected':''}>${label}</option>`).join('')}</select></label><label>Canvas<select id="orientation"><option value="landscape" ${design.orientation==='landscape'?'selected':''}>Landscape · 1920 × 1080</option><option value="portrait" ${design.orientation==='portrait'?'selected':''}>Portrait · 1080 × 1920</option></select></label><label>Background<select id="frame"><option value="filled" ${design.frame==='filled'?'selected':''}>Filled</option><option value="none" ${design.frame==='none'?'selected':''}>Transparent overlay</option></select></label></aside><section><div id="preview" class="preview">${result.graphic.svg}</div><p class="caption">${result.graphic.width} × ${result.graphic.height} · actual rendered SVG · ${design.frame==='none'?'checkerboard is preview only':'filled background'}</p></section><aside class="panel"><h2>Make it yours</h2><label>Title<input id="graphic-title" maxlength="80" value="${esc(design.title)}" placeholder="On the course"></label><label>Accent<input id="accent" type="color" value="${design.accent}"></label><label>Background color<input id="background" type="color" value="${design.background}"></label><label>Text<input id="foreground" type="color" value="${design.foreground}"></label><button id="keep-graphic" class="primary full">Keep for ExportGraphics</button><p class="caption">Keeps this exact graphic. Later design changes won’t change what you kept.</p><h3>${context.graphics.length} kept versions</h3><div class="saved">${context.graphics.map(address=>{const c=model.value(address);return `<button data-version="${esc(address)}">${esc(c.title)}<br><small>${c.graphic.width} × ${c.graphic.height} · ${c.hash.slice(0,8)}</small></button>`;}).join('')}</div></aside></div>`;
  $('graphic-disc').onchange=()=>{const selectedDisc=$('graphic-disc').value;run(async()=>{await model.patch({selectedDisc});await drawCreate();});};
  for(const id of ['preset','orientation','frame','accent','background','foreground','graphic-title'])$(id).onchange=()=>{const key=id==='graphic-title'?'title':id,next=$(id).value;run(async()=>{await model.patch({design:{...model.context.design,[key]:next}});await drawCreate();});};
  ...
  document.querySelectorAll('[data-version]').forEach(button=>button.onclick=()=>run(async()=>{const capture=model.value(button.dataset.version);await model.patch({selectedDisc:capture.source.disc,design:capture.design});await drawCreate();say('Reopened that kept design. Its original capture stays unchanged.');}));
}
async function drawExport(){
  ...
  $('app').innerHTML=intro('Take the graphic with you.','Inspect the captured version, then download those pixels or their SVG source.')+`<div class="workspace two"><aside class="panel"><h2>Kept graphics</h2><button id="import-kept" class="primary full" ${active.inspect().kind==='test'?'disabled':''}>Import from CreateGraphics</button><button id="load-example" class="full">Use a seeded example</button><p class="caption">Import is explicit. Test sandboxes never read your interactive captures.</p><div class="saved">${addresses.map(address=>{const c=model.value(address);return `<button data-capture="${esc(address)}" class="${address===chosenCapture?'selected':''}">${esc(c.title)}<br><small>${c.graphic.width} × ${c.graphic.height} · ${c.hash.slice(0,8)}</small></button>`;}).join('')}</div></aside><section><div id="preview" class="preview">${capture?capture.graphic.svg:'<p class="empty">Your next exported graphic goes here.<br>Import a kept version or try the seeded example.</p>'}</div><p class="caption">${capture?`${capture.graphic.width} × ${capture.graphic.height} · captured ${esc(capture.design.frame==='none'?'transparent overlay':'filled background')}`:'No graphic selected.'}</p></section><aside class="panel"><h2>Export this version</h2>${capture?`<p><strong>${esc(capture.title)}</strong><br><small>SHA-256 · ${capture.hash.slice(0,16)}…</small></p>`:''}<label>Format<select id="format"><option value="png">PNG · image</option><option value="svg">SVG · editable vector</option></select></label><button id="download" class="primary full" ${capture?'':'disabled'}>Download graphic</button><p class="caption">Uses this capture. PNG is rasterized by this browser at the dimensions above.</p><h3>Export results</h3><div class="history">${context.exports.slice().reverse().map(address=>{const r=model.value(address);return `<article><strong>${esc(r.status)}</strong><br>${esc(r.format.toUpperCase())} · ${r.byteLength??0} bytes<br>${esc(r.filename??r.error)}<br><code>${esc(r.hash??'')}</code></article>`;}).join('')||'<small>No exports yet.</small>'}</div></aside></div>`;
  $('import-kept').onclick=()=>run(async()=>{let count=0;for(const offer of JSON.parse(localStorage.getItem(mailbox)??'[]'))if(await model.importCapture(offer))count++;await drawExport();say(`${count} new captured graphic${count===1?'':'s'} imported. Existing versions preserved.`);});
  $('load-example').onclick=()=>run(async()=>{const graphic=await model.render(model.context.selectedDisc,model.context.design);await model.capture(graphic,active.name);await drawExport();say('Seeded Spotlight example · produced through the same card Calculations.');});
  ...
  $('download').onclick=()=>{const selected=capture,address=chosenCapture,format=$('format').value;run(async()=>{
    ...
    try {
      ...
      const hash=await sha256(blob),filename=`discstudio-${selected.title.toLowerCase().replace(/[^a-z0-9]+/g,'-')}-${selected.graphic.width}x${selected.graphic.height}-${hash.slice(0,8)}.${format}`;
