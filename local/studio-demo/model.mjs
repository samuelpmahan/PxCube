...
import { calculations, defaultDesign } from './graphics.mjs';
import { sha256 } from './renderer/src/media.js';

const prefix='ds.px.studio.demo.';
export const captureIdentity=capture=>JSON.stringify([capture.source.mount,capture.source.graphic,capture.hash]);
const frozen=value=>Object.freeze(structuredClone(value));
const install=pxc=>{for(const [name,fn] of Object.entries(calculations)) if(!pxc.entries().some(([address])=>address===name))pxc.set(name,new Part(fn));};
...
export async function openDemo(storage) {
  ...
  const rendered=new Map(),queries=new Map();
  async function render(discAddress,design) {
    ...
    const fields=await compose(base+'fields','fn.studio.graphicFields',{disc:discAddress,resolved});
    const art=await compose(base+'art','fn.studio.graphicArt',{disc:discAddress,art:disc.art??`ds.px.art.${disc.id}`});
    const definition=await compose(base+'definition','fn.studio.graphicDesign',{design:request});
    const effective=await compose(base+'effective','fn.studio.cardTokens',{design:definition,disc:discAddress});
    const preset=await compose(base+'preset','fn.studio.cardPreset',{design:definition,effective});
    const card=await compose(base+'card','fn.studio.card',{fields,art,preset});
    const frame=await compose(base+'frame','fn.studio.frame',{design:request,tokens:effective});
    const scene=await compose(base+'scene','fn.studio.scene',{card,frame,design:request});
    const address=await compose(base+'graphic','fn.studio.graphic',{scene,frame});
    const result={address,discAddress,designAddress:request,fieldsAddress:fields,resolvedAddress:resolved,graphic:value(address)};
  ...
  return {
    experience,pxc,value,render,capture,importCapture,patch,
