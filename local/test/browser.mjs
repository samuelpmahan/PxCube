...
const evidence=path.join(root,'evidence/local-pages'), checks=[],errors=[];
...
try {
 ...
 for(const [mode,url] of [['local','http://127.0.0.1:4321/'],['project-prefix',`http://127.0.0.1:${staticHost.address().port}/PxCube/`]]) {
  ...
  const frame=page.frameLocator('#thing');await frame.locator('article[data-mount="mock.shelf.1"]').waitFor();
  await frame.locator('article[data-mount="mock.shelf.1"] input').fill('Morning practice');
  await frame.locator('article[data-mount="mock.shelf.1"] button').click();
  await frame.locator('#new-run').click();
  assert.equal(await frame.locator('article[data-mount="mock.shelf.1"] input').inputValue(),'Morning practice');
  ...
  assert.equal(model.runs['mock.shelf.1'].value.sc.draft.name,'Morning practice');
  ...
  checks.push({mode,runId:record.runId,checks:['actual neat board is served','hello opens','visible save changes actual mounted value','new iteration preserves old run','three visible smoke assertions pass','Back/reopen retains worlds','page reload retains worlds','packaging report names both successful apps']});
 }
