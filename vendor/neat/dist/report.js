...
const refList = (refs) => refs.length
...
function matrixTab(matrix, items, board) {
    if (!matrix)
        return `<section id="tab-matrix" class="tab-panel" role="tabpanel"><p class="empty">No mGM matrix supplied yet. This view accepts one without changing the base work-item model.</p></section>`;
    const rows = matrix.units.map((unit) => {
        ...
        return `<tr data-unit="${attr(unit.id)}"><th scope="row"><b>${esc(unit.title)}</b><code>${esc(unit.target.kind)}:${esc(unit.target.identity)}</code><span>${esc(unit.inspectableResult)}</span>${observed}</th>${["minimal", "goldilocks", "maximal"].map((scope) => {
            const cumulative = scopeRank[scope] <= scopeRank[target];
            const requirements = ["minimal", "goldilocks", "maximal"]
...
function dependenciesTab(items, board) {
    ...
    const edges = board.graph.edges.filter((edge) => positions.has(edge.item) && positions.has(edge.prerequisite));
    const lines = edges.map((edge) => {
        const from = positions.get(edge.prerequisite);
        const to = positions.get(edge.item);
        const dx = to.x - from.x;
        const dy = to.y - from.y;
        const scale = Math.min(1, 78 / Math.max(1, Math.abs(dx)), 16 / Math.max(1, Math.abs(dy)));
        const sx = from.x + dx * scale;
        const sy = from.y + dy * scale;
        const tx = to.x - dx * scale;
        const ty = to.y - dy * scale;
...
export function renderReport(board, items, options = {}) {
  ...
  </style></head><body><h1>${esc(title)}</h1><nav class="tabs" role="tablist"><button class="tab" role="tab" aria-selected="true" aria-controls="tab-board" data-tab="tab-board">Board</button><button class="tab" role="tab" aria-selected="false" aria-controls="tab-matrix" data-tab="tab-matrix">Matrix</button><button class="tab" role="tab" aria-selected="false" aria-controls="tab-dependencies" data-tab="tab-dependencies">Dependencies</button></nav>${boardTab(items, board)}${matrixTab(options.matrix, items, board)}${dependenciesTab(items, board)}<script>document.querySelectorAll('.tab').forEach(function(b){b.addEventListener('click',function(){document.querySelectorAll('.tab').forEach(function(x){x.setAttribute('aria-selected',String(x===b));});document.querySelectorAll('.tab-panel').forEach(function(x){x.classList.toggle('active',x.id===b.dataset.tab);});});});document.getElementById('tab-board').classList.add('active');</script></body></html>`;
}
